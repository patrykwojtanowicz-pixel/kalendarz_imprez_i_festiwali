...
function kif_event_meta_cb($post){
    ...
    <style>.kif-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.kif-grid .full{grid-column:1/-1}</style>
    <div class="kif-grid">
      <p><label>Data i godzina</label><br><input type="datetime-local" name="kif_date" value="<?php echo esc_attr($date); ?>" style="width:100%"></p>
      <p><label>Miasto</label><br><input type="text" name="kif_city" value="<?php echo esc_attr($city); ?>" style="width:100%"></p>
      <p><label>Lokalizacja</label><br><input type="text" name="kif_venue" value="<?php echo esc_attr($venue); ?>" style="width:100%"></p>
      <p><label>Cena biletu (np. 199 lub „od 179”)</label><br><input type="text" name="kif_price" value="<?php echo esc_attr($price); ?>" style="width:100%"></p>
      <p class="full"><label>Gatunek(i) (przecinki)</label><br><input type="text" name="kif_genre" value="<?php echo esc_attr($genre); ?>" style="width:100%"></p>
      <p class="full"><label>Link do biletów</label><br><input type="url" name="kif_ticket" value="<?php echo esc_attr($ticket); ?>" style="width:100%"></p>
      <p class="full"><label>Line-up (jeden artysta na linię)</label><br><textarea name="kif_lineup" rows="6" style="width:100%"><?php echo esc_textarea($lineup); ?></textarea></p>
      <p class="full"><label>Headlinerzy (przecinki)</label><br><input type="text" name="kif_headliners" value="<?php echo esc_attr($heads); ?>" style="width:100%"></p>
      <p><label>Tryb lineup’u</label><br>
        <select name="kif_lineup_mode" style="width:100%">
          <?php $opts=['full'=>'Pełna lista','full_headliners'=>'Pełna lista z headlinerami','headliners_only'=>'Tylko headlinerzy','days_stages'=>'Pełna lista z podziałem na dni i sceny','timetable'=>'Timetable (graficzny)']; $cur=$mode?:'full'; foreach($opts as $k=>$v){ echo '<option value="'.esc_attr($k).'" '.selected($cur,$k,false).'>'.esc_html($v).'</option>'; } ?>
        </select>
      </p>
      <p><label>Timetable (JSON 24h)</label><br><textarea name="kif_timetable" rows="6" style="width:100%"><?php echo esc_textarea($tt); ?></textarea></p>
    </div>
<?php }
